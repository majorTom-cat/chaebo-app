...
_ENV_MEMO = {}  # 분석 1회(서브프로세스) 내 같은 x 재계산 방지 — id+len 키(x 는 분석 내내 참조돼 id 안정)


def _bass_envelope(x, sr, hop_ms=8):
    """8ms RMS envelope(정규화 0~1) — 표시 파형과 같은 진폭 근거. 반환:(env, 프레임초).
    한 분석 안에서 onset·quiet_gaps·refine·merge 가 같은 x 로 3~4회 부르던 걸 memoize(코드검사 2026-07-17)."""
    key = (id(x), len(x), sr, hop_ms)
    ...
    out = (e / (float(e.max()) or 1.0), hop / sr)
    _ENV_MEMO[key] = out
    return out
...
def detect_notes_bp(x, sr, work_dir):
    ...
    try:  # 예외(ONNX 로드 실패·손상 입력) 시에도 임시 wav 삭제(코드검사 2026-07-17: try/finally 부재로 잔존)
...
_HARMONY_CHROMA_MEMO = {}  # 화성 chroma(frames) — 키추정·코드추정이 같은 4스템+CQT를 각각 돌던 걸 공유(코드검사 2026-07-17)


def _harmony_chroma(stems_dir):
    """화성 스템(guitar/piano/other/vocals, 드럼·베이스 제외) 합산 → chroma_cqt(hop 2048, frames).
    키 12-벡터는 이 chroma 의 시간평균이라 파생 가능 — 로드+CQT 를 한 번만(stems_dir memoize). 없으면 None."""
    key = str(stems_dir)
    if key in _HARMONY_CHROMA_MEMO:
        return _HARMONY_CHROMA_MEMO[key]
    try:
        import librosa
    except Exception:  # noqa: BLE001
        _HARMONY_CHROMA_MEMO[key] = None
        return None
    sig = None
    ...
    ch = None if (sig is None or len(sig) < 22050) else librosa.feature.chroma_cqt(y=sig, sr=22050, hop_length=2048)
    _HARMONY_CHROMA_MEMO[key] = ch
    return ch


def _chroma_key_profile(stems_dir):
    """화성 스템 chroma 평균 = 키 추정용 12-벡터. 베이스 음분포만으로는 근음·5도에 쏠려 딸림음을 으뜸음으로
    오검(실증: 곡14 G↔C) — 화성 전체를 함께 봐 상쇄. 스템 없으면 None."""
    ch = _harmony_chroma(stems_dir)
    return ch.mean(axis=1) if ch is not None else None
...
def chroma_chords(stems_dir, notes, slots, bpm, offset, bar_slots, key=None):
    ...
    try:
        ...
        return None
    # 화성 스템(guitar/piano/other/vocals, 베이스 제외) chroma — 키추정과 공유(코드검사 2026-07-17: 종전 중복 로드+CQT)
    chroma = _harmony_chroma(stems_dir)
    if chroma is None:
        return None
    hop = 2048
    tpf = hop / 22050.0
